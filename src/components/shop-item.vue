  ...
  <div class="item">
    <div>
      <div class="item-img"><img :alt="item.name"
                                 :src="item.sku_info[0].ali_image+'?x-oss-process=image/resize,w_206/quality,Q_80/format,webp'"
                                 style="opacity: 1;">
      </div>
      <h6>{{item.name}}</h6>
      ...
      <div class="item-btns clearfix">
        <span class="item-gray-btn"><router-link tag="a"
                                                 :to="{path:'/item',query:{itemId:item.sku_info[itemIndex].sku_id}}">查看详情</router-link>  </span>
      ...
      <div class="item-cover">
        <router-link tag="a" :to="{path:'/item',query:{itemId:item.sku_info[itemIndex].sku_id}}">
        </router-link>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      addCarPanelHandle(data) {
        let carData = {info: data, count: 1}
        this.$store.commit('addCarPanelDate', carData)
      }
    }
