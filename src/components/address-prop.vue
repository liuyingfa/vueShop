    ...
    <div class="module-dialog clear module-dialog-address module-dialog-show">
      <div class="dialog-panel">
        <div class="topbar">
          ...
          <span class="dialog-close" @click="closeProp">x</span>
        </div>
                ...
                <div class="address-form">
                  ...
                  <div class="module-form-row">
                    <div class="form-item-v3" :class="{'form-invalid-item':phoneError}">
                      <input type="text" placeholder="手机号" class="js-verify" v-model="receive.phone"
                             @blur="inspectPhone">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row clear">
                    <div class="form-item-v3 area-code-w fn-left form-valid-item">
                      <input type="text" placeholder="区号（可选）" class="js-verify js-address-area-code"
                             v-model="receive.areaCode">
                      <div class="verify-error"></div>
                    </div>
                    <div class="form-item-v3 telephone-w fn-right form-valid-item">
                      <input type="text" placeholder="固定电话（可选）" class="js-verify js-address-telephone"
                  ...
                  <div class="module-form-row clear">
                    ...
                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="receive.countyId">
                        ...
                        <option :value="country.area_id" v-for="country in countryList">{{country.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row">
                  ...
                  <div class="dialog-blue-btn big-main-btn js-verify-address " :class="{'disabled-btn':!isValidate}" @click="receiveSubmit" >
                    <a >保存</a>
                  </div>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      inspectPhone() {
        if (this.receive.phone.length != 9) {
      ...
      checkDefault() {
        ...
        console.log(this.receive.default)
      },
      receiveValidate() {
        if (this.receive.name && !this.phoneError && this.receive.province && this.receive.city && this.receive.countyId) {
      ...
      receiveSubmit(){
        if(this.isValidate){
          this.$store.commit('receiveSubmit',this.receive)
          this.$emit('close')
        }
      }
    }
