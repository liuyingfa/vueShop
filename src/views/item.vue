      ...
      <div class="item-box">
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumbnail">
              <ul>
                <li :class="{'on':index==imgIndex}" @click="tabIndex(index)" v-for="item,index in itemInfo.ali_images">
                  <img :src="item + '?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
            <div class="thumb">
              <ul>
                <li :class="{'on':index==imgIndex}" v-for="item,index in itemInfo.ali_images"><img
                  :src="item + '?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
          </div>
        </div>
        <div class="banner">
            ...
            <div class="sku-dynamic-params clear">
              ...
              <ul class="params-colors">
                <li :class="{'cur':color.id == $route.query.itemId}" v-for="color,index in itemInfo.sku_list ">
                  <router-link tag="a" :to="{path:'/item',query:{itemId:color.id}}" :title="color.color">
            ...
            <div class="sku-dynamic-params clear">
              ...
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  <span class="down" @click="subCount" :class="{'down-disabled': count<1}">-</span>
                  <span class="num">{{count}}</span>
                  <span class="up " :class="{'up-disabled': count>itemInfo.limit_numi}" @click="addCout">+</span>
                </div>
              </div>
            </div>
          </div>
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              <span class="blue-title-btn js-add-cart" @click="addCar"><a>加入购物车</a></span>
...
<script>
  ...
  export default {
    ...
    computed: {
      itemInfo() {
        let itemInfo = this.itemsList.filter((item) => {
