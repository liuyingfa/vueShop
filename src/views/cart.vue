          ...
          <div v-else>
            ...
            <div class="cart-table">
              <div class="cart-group">
                <!--购物列表-->
                <div class="cart-top-items" v-for="cartInfo  in carPaneData">
                  <div class="cart-items">
                    <div class="items-choose">
                      <span class="blue-checkbox-new" :class="{'checkbox-on':cartInfo.checked}" @click="checkedGoodsHandle(cartInfo.sku_id)"><a></a></span>
                    </div>
                    <div class="items-thumb">
                      <img :src="cartInfo.ali_image">
                      <a href="javascript:;" target="_blank"></a>
                    </div>
                    <div class="name hide-row" >
                      <div class="name-table">
                        <a href="javascript:;" target="_blank"> {{ cartInfo.title }}</a>
                        <ul class="attribute">
                          <li>{{ cartInfo.spec_json.show_name }}</li>
                        </ul>
                      </div>
                    </div>
                    <div class="operation">
                      <a class="items-delete-btn"  @click="delCarPanelHandle(cartInfo.sku_id)"></a>
                    </div>
                    <div class="subtotal">¥ {{cartInfo.price * cartInfo.count}}.00</div>
                    <div class="item-cols-num">
                      <div class="select js-select-quantity">
                        <span class="down" @click="subCountHandle(cartInfo.sku_id)">-</span>
                        <span class="num"> {{cartInfo.count}}</span>
                        <span class="up" @click="plusCountHandle(cartInfo.sku_id)">+</span>

                      </div>
                    </div>
                    <div class="price">¥ {{cartInfo.price}}.00</div>
                  </div>
                </div>
          ...
          <div class="cart-bar-operation">
            <div>
              <div class="choose-all js-choose-all">
                <span class="blue-checkbox-new" :class="{'checkbox-on':checkAll}" @click="checkedAllGoods(checkAll)"><a></a></span>
                全选
              </div>
              <div class="delete-choose-goods" @click="delCheckHandle">删除选中的商品</div>
            </div>
          </div>
          <div class="shipping">
           ...
           <router-link  class="jianguo-blue-main-btn big-main-btn js-checkout " tag="span" to="/checkout" :class="{'disabled-btn':checkCount<=0}"><a>现在结算</a></router-link>
          </div>
        </div>
      </div>
    </div>
...
<script>
    export default {
      ...
      methods:{
        ...
        plusCountHandle(id){
          this.$store.commit('plusCountHandle',id)
        },
        subCountHandle(id){
          this.$store.commit('subCountHandle',id)
        },
        checkedGoodsHandle(id){
          this.$store.commit('checkedGoodsHandle',id)
        },
        checkedAllGoods(checkAll){
          this.$store.commit('checkedAllGoods',checkAll)
        },
        delCheckHandle(){
