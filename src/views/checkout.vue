            ...
            <div class="address-common-table js-multiple-address-panel">
              <ul class="address-item-list clear js-address-item-list">
                <li class="js-choose-address "
                    :class="{'selected-address-item': receiveDef ==index }"
                    v-for="info,index in receiveInfo"
                    @click="chooseAddres(index)">
                ...
                <li class="add-address-item js-add-address" @click="showProp">
                  <p>使用新地址</p>
                </li>
              </ul>
            </div>
          ...
          <div class="invoice-detail"> 发票抬头：
            <div class="radio-box">
              <label>
                ...
                <span class="blue-radio " :class="{'blue-radio-on':Invoices.personal}"
                      @click="checkPersonal(true)"><a></a></span> 个人
              </label>
              <label>
                <input type="radio" class="hide">
                <span class="blue-radio" :class="{'blue-radio-on':!Invoices.personal}"
                      @click="checkPersonal(false)"><a></a></span> 单位
              </label>
            </div>
            <div class="module-form-row form-item fn-hide js-invoice-title">
              <div class="module-form-item-wrapper no-icon small-item">
                <i v-if="!Invoices.text">请填写公司发票抬头</i>
                <input type="text" class="js-verify" v-model="Invoices.text">
              </div>
            </div>
          </div>
        ...
        <div class="box-inner ui-goods-cart">
          ...
          <div class="cart-table">
            <div class="cart-group js-cart-group">
              <div class="cart-items" v-for="goods in checkedGoods">
                ...
                <div class="name hide-row">
                  <div class="name-cell">
                    <a href="javascript:;" :title="goods.title+'('+goods.spec_json.show_name+')'" target="_blank">{{
                      goods.title+'('+goods.spec_json.show_name+')'}}</a>
                  </div>
                </div>
                <div class="subtotal">
        ...
        <div class="box-inner">
          <div class="last-payment clear">
            <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout"
                  @click="submitOrderHandle"> <a>提交订单</a> </span>
            <span class="prices fn-right">应付金额： <em>¥ {{checkPrice+freight}}.00</em></span>
          </div>
        </div>
      </div>
    </div>
    <address-prop v-if="popShow" @close="closeProp"></address-prop>
  </div>
</template>

<script>
  import addressProp from '@/components/address-prop'
  ...
  export default {
    ...
    methods: {
      ...
      checkPersonal(checked) {
        this.Invoices.personal = checked
      },
      submitOrderHandle() {
        if (!this.Invoices.personal && !this.Invoices.text) return
        //地址信息
        let receive = this.receiveInfo[this.receiveDef]
        if (this.Invoices.personal) {
          this.Invoices.text = '个人'
        }
        let iDate = new Date()
        ...
        let data = {
          ...
          invoiceText: this.Invoices.text,
